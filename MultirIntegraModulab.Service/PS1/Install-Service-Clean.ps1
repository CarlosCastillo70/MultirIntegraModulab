# Script d'instalacio del Windows Service MultirIntegraModulab
...
$ServiceDescription = "Servei per executar periodicament la integracio de mostres Modulab i la revisio de vigencia de diagnostics"
...
Write-Host " Instalacio del Windows Service" -ForegroundColor Cyan
Write-Host "===================================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($existingService)
{
	Write-Host "El servei ja existeix. Aturant i desinstalant..." -ForegroundColor Yellow
	...
	# Desinstallar servei existent
	sc.exe delete $ServiceName | Out-Null
	Start-Sleep -Seconds 2
}

Write-Host "Instalant servei..." -ForegroundColor Green
...
# Configurar descripcio
sc.exe description $ServiceName "$ServiceDescription" | Out-Null

# Configurar recuperacio automatica en cas de fallada
sc.exe failure $ServiceName reset= 86400 actions= restart/60000/restart/60000/restart/60000 | Out-Null

Write-Host "Servei instalat correctament!" -ForegroundColor Green
Write-Host ""
...
if ($startService -eq 'S' -or $startService -eq 's')
{
	...
	{
		Write-Host "El servei no s'ha pogut iniciar. Reviseu els logs d'esdeveniments." -ForegroundColor Yellow
	}
}
